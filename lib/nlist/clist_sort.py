...
class clist:
    def __init__(self, r_cut, r_buff=0.5):
        ...
        self.cu_cell_list, self.cu_cell_map = self._gen_func()
    ...
    def update(self):
        with cuda.gpus[self.gpu]:
            ...
            self.cu_cell_list[self.bpg_part, self.tpb](self.system.d_x, self.system.d_box, self.d_ibox,
                                                       self.d_cells, self.d_cell_counts)
            cupy.cumsum(self.d_cell_counts, out=self.d_cell_counts)
            self.d_cell_list = cupy.argsort(self.d_cells)
    ...
    def _gen_func(self):
        ...
        def _gen_cell_list():
            ...
            @cuda.jit(void(nb_float[:, :], nb_float[:], int32[:], int32[:], int32[:]))
